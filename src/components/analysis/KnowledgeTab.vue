...
const { filteredItems, loading, statistics, categories, filter } = storeToRefs(knowledgeStore)
...
function toggleExpand(id: number) {
  if (expandedId.value !== id) {
    ...
    if (!isBrowserEnvironment()) {
      ...
      if (idx !== -1) knowledgeStore.items[idx] = { ...knowledgeStore.items[idx], viewCount: knowledgeStore.items[idx].viewCount + 1 }
    }
  } else {
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        >
          ...
          >
            ...
            >
              <p class="whitespace-pre-wrap text-sm text-gray-700 dark:text-gray-300" data-testid="knowledge-content">{{ item.content }}</p>
