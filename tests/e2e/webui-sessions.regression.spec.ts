 ...
 *
 * 策略：将 api-server.json 写入 SYSTEM userData（Electron 实际读取的位置），
 * 因为 app.setPath('userData') 在当前构建中不生效。
 */
async function startAppWithApiServer() {
  ...
  console.log('[Regression] 已写入系统 api-server.json:', apiConfig)

  const app = await launchApp({ userDataDir, startupWaitTime: 8000 })
  await waitForApiServer(60_000)
...
test.describe('Web UI 会话数据加载回归测试', () => {
  ...
  test.beforeAll(async () => {
    ...
    const result = await startAppWithApiServer()
    app = result.app
  ...
  test('REG-002: 登录获取 token', async ({ request }) => {
    ...
    console.log('[REG-002] login response:', { success: body.success, hasToken: !!body.data?.token })
  ...
  test('REG-005: GET /api/v1/sessions 本地请求无需 token 返回 200', async ({ request }) => {
    // smartAuth allows local (127.0.0.1) requests without token by design —
    // the Electron desktop app calls this endpoint without a Bearer token.
    // Only non-local (remote browser) requests require authentication.
    const res = await request.get(`${API_BASE}/api/v1/sessions`)
    console.log('[REG-005] status:', res.status())
    expect(res.status()).toBe(200)
  })

  test('REG-006: /api/webui/sessions 使用有效 token 返回 200', async ({ request }) => {
    // Get a fresh valid token by logging in
    const loginRes = await request.post(`${API_BASE}/api/webui/auth/login`, {
      data: { username: 'admin', password: 'admin123' },
    })
    const loginBody = (await loginRes.json()) as any
  ...
  test('REG-014: GET /api/v1/sessions 无效 token 返回 401', async ({ request }) => {
    // /api/v1/* routes: local IP (127.0.0.1) is allowed without token (Electron desktop),
    // but any request with an invalid/expired token must be rejected with 401.
    const res = await request.get(`${API_BASE}/api/v1/sessions`, {
  ...
  test('REG-015: POST /api/v1/sessions/:id/sql 参数化查询支持 params 数组', async ({ request }) => {
    ...
    const body = (await res.json()) as any
    console.log(
      '[REG-015] sql with params status:',
      res.status(),
      'tables:',
      body.data?.map((r: any) => r.name)
    )
    expect(res.status()).toBe(200)
    expect(body.success).toBe(true)
    expect(Array.isArray(body.data)).toBe(true)
  ...
  test('REG-019: POST /api/v1/sessions/:id/sql 无效 token 返回 401', async ({ request }) => {
    ...
    // /api/v1/* routes: invalid token must be rejected with 401
    expect(res.status()).toBe(401)
  })
  ...
  test('REG-028: GET /api/v1/nlp/pos-tags 无效 token 返回 401', async ({ request }) => {
  ...
  test('REG-029: GET /nlp/word-frequency 返回词频数据对象', async ({ request }) => {
    ...
    const sessions = ((await sessionsRes.json()) as any).data ?? []
    if (!sessions.length) {
      console.log('[REG-029] 无会话，跳过')
      return
    }
    const id = sessions[0].id
    const res = await request.get(
      `${API_BASE}/api/v1/sessions/${id}/nlp/word-frequency?locale=zh-CN&topN=50&minCount=2`
    )
    const body = (await res.json()) as any
  ...
  test('REG-030: GET /sql/schema 返回表结构数组', async ({ request }) => {
    ...
    const body = (await res.json()) as any
    console.log(
      '[REG-030] schema tables:',
      body.data?.map((t: any) => t.name)
    )
    expect(res.status()).toBe(200)
    expect(body.success).toBe(true)
    expect(Array.isArray(body.data)).toBe(true)
  ...
  test('REG-032: GET /messages/before/:id 返回消息数组', async ({ request }) => {
    ...
    if (!sqlBody.data?.length) {
      console.log('[REG-032] 无消息数据，跳过')
      return
    }
  ...
  test('REG-035: GET /api/v1/assistants 无效 token 返回 401', async ({ request }) => {
  ...
  test('REG-036: GET /api/v1/assistants 每个助手有 id/name/systemPrompt 字段', async ({ request }) => {
    ...
    // /api/v1/* routes: invalid token must be rejected
    expect(res.status()).toBe(401)
  })

  test('REG-037: GET /api/v1/assistants/general_cn 无效 token 返回 401', async ({ request }) => {
  ...
  test('REG-038: GET /api/v1/assistants/nonexistent 无效 token 返回 401', async ({ request }) => {
  ...
  test('REG-054: GET /api/v1/assistants 无效 token 返回 401', async ({ request }) => {
    const res = await request.get(`${API_BASE}/api/v1/assistants`, {
      headers: { Authorization: 'Bearer invalid-token-xyz' },
    })
  ...
  test('REG-055: GET /api/v1/assistants/:id 无效 token 返回 401', async ({ request }) => {
  ...
  test('REG-057: GET /stats/weekday-activity 每项有 weekday/count 字段', async ({ request }) => {
    ...
    expect(res.status()).toBe(200)
    expect(body.data.length).toBe(7)
    // 每项应有 weekday(1-7，周一到周日) 和 messageCount 字段
    for (const item of body.data) {
      expect(typeof item.weekday).toBe('number')
      expect(typeof item.messageCount).toBe('number')
      expect(item.weekday).toBeGreaterThanOrEqual(1)
      expect(item.weekday).toBeLessThanOrEqual(7)
    }
  })
  ...
  test('REG-060: GET /nlp/pos-tags 无效 token 返回 401', async ({ request }) => {
    const res = await request.get(`${API_BASE}/api/v1/nlp/pos-tags`, {
      headers: { Authorization: 'Bearer invalid-token-xyz' },
    })
